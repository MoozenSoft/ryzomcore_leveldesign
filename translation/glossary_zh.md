...
## 批次 3B（words 四族：skill/outpost/title/place）定译

**流程**：无自建脚本——直接在官方 diff 文件上逐行产出译文（本次按列拼装以保 hash/id 列零转录误差，翻译内容全部人工产出）；基准 `translated/<f>_words_en.txt` 按 id 对齐；尾两行未译标记删除后 merge。**off-by-one 尾垃圾行未复现**：本批 4 族目标文件已以 header-only 孤儿形式存在（3A 产物），merge 走"追加进已有文件"路径，绕开 bootstrap——3C+ 若目标文件同样预存则无需剥尾。place_words_zh 结构随 wk/diff 母本（8 列），en 文件为过时 4 列布局，**place 的 header==en 断言不成立（实际==wk）**；其余三族 header 与 en 逐字节一致。

**outpost/place 前哨建筑后缀表**：Farm 农场 / Workshop 工坊 / Border Post 边境哨站 / Trade Post 商栈 / Trading Post 贸易站 / Stronghold 要塞 / Research Center 研究中心 / Diplomatic Outpost 外交前哨站 / Magic Pole 魔法图腾 / Hall 大厅（前哨站大厅）；placeholder：Constructible space 可建造地块、Revert to an empty space 还原为空地。钻探机族：Tree-Bore 钻木机 / Big Trepan 大钻机 / Superior 高级 / Hardy 加固 / Rapid 高速 / Crystal 水晶 / Offering for X「X供品」、desc 制造原材料 + （双倍）消耗品（速度 x1.5|x2）——尾随空格逐列随 en（driller 16 行双倍消耗品无尾空格、其余有）。佣兵族：Militiamen 民兵 / skirmishers 散兵 / battlers 斗士 / heavy warriors 重装武士 / protectors 护卫 / gunfighters 手枪射手 / gunners 步枪手 / artillery shooters 炮手 / harriers 掠袭 / fighters 战士 / elementalists 元素师 / disturbers 扰乱者 / magicians 法师；"A squad of eight {档} mercenaries"→八人{档}佣兵小队（档=新手/学徒/熟手/进阶/专家/大师 c2 段位），desc 句号有无随 en。

**place 地名策略**（沿用 2.8 定表）：region/continent 行用 PvP 区域名终表（威严花园/直觉之城/灵液深渊/上部沼泽…39 名）；城市 place_* 保拉丁（Yrkanis/Pyr/Zora/Fairhaven…，Arena 亦留）；lieudit 描述词直译、生僻虚构专名/物种名保拉丁（Mullgrove/Slavenhurst/Stainmoor/Malmont/Blackburn/Yelkgrove/Saprun/Darkring/Stalia/Ryecliff/Wooky/Ginti/Qai-Du/Sai-Shun/Gu-Qin/Zo-Kian/Ni-Jing/Kipee/Clopper/Varynx/Torbak/Kipucka/Mossok/Leron/Cryton/Tryde/Dante/Almati/Aelius/Olkern/Rigan/Hartley/Hobelen/Frogmore/Corrie Finley/Farely/Tupile/Windermeer/Highback…；通词 -mull 岬、-hurst 林岗、-dell 小谷、-stock 枯木、-mere 湖、-gate 门、Tor 瀑峰、Billabong 水潭、Henge 石阵、Gully 沟壑、Scarp 断崖、Creek 溪）；"< landmark > <建筑>" 复合名与 outpost 族同词同译；welcomer_*→X 卫兵队长/行政官/贤者/官员（r1 官职）；ma/tr/zo_*_quarter→X 队长/监工/信使/贤者/中士/卫兵队长/行政官 营房；indoors→培训师（r1 Trainer）；调试行 "If you see this text, it's a bug: <id>。" 保 id 原样；`<GEN>xxx` 55 行与 `place de test` 法文调试行按语义/逐字节规则处理（<GEN> 原样、法文调试意译）；en 空名行保空。

**title 族**：690 行与批次 2.8 rem_zh_all.json 同键——621 行 en.uxt 值与 words en 一致者逐字沿用；69 行 words 值与 uxt 漂移者（gn_class 的 Warrior/Healer vs Shooter/Magician、Crusher/Whirlwind 等玩家称号风味词、fct_tb/tc 对调、Elite Pikeman 等）**按 words en 值重译**（武士/治疗师、碎骨者/旋风手、精英长矛手…）。新定族：welcomer X Kami/Karavan 迎宾员、ambassador X Kami/Karavan 大使、agent Karavan 探员、officer Karavan 军官、Altar X Kami/Karavan 祭坛、Stele of Valor 英勇石碑、Knowledge Root/Column 知识之根/柱、Guard 卫兵队长、Sentinel 哨卫、Warlord 战争领主、Warder 胶质看守、Chaplain 司祭、Preacher 布道者、Envoy 特使/Emissary 使者/Herald 使者/Representative 代表/Seeker 寻道者、Bonze 僧侣（r4）、High Servant 高阶侍从、Quartermaster 军需官、Commander in Chief 总司令、Trooper 士兵、War Vessel 战舰、Medic 军医、Drill 钻机（小型 Kami/Karavan 钻机）、Swarmheader 蜂群首领、Aggressor 侵袭者、Raider 突袭者、Witch Doctor 巫医、High Chief 大酋长；Kitin 猎手阶梯（新手/大师 Kitin 猎手、Kitin 根除者/屠戮者/歼灭者）；匪首绰号 r1 式（人名+描述词：Aen 沙漠之刃、Sirgio 凶徒、Lixie 狂怒者、Pei-Ziao 祸害、Dante 挑衅者、Pei-Ruz 腐烂者、O'Darghan 狂人、Weee'ly 傻瓜）；Henchman/Henchwoman→X 的随从/女随从；women_name 列：中文无语法性别，同名列照抄、真性别词用"女X"前缀（女皇/女王/公主/女祭司/女精英长矛手/女剑刃大师/女酒保…）；`$fct_ranger_leader$` 占位符原样；To New Horizons 公司名留拉丁；fct_* 括号用全角、前导空格保留。

**skill 族**：武器根复用 uxt 成品表（锤/杖/矛/长矛/斧/剑/匕首/手枪/弓手枪/弓式步枪/连发投射器/投射器/步枪；生命X=Living、波动X=Waving、电击X=Electric、燃烧X=Burning、单手/双手、近战/远程）；等级 Advanced/Expert/Master→进阶/专家/大师（"Master <武器>"→大师<武器>）；分类路径 desc：Magic targeted→目标指向魔法、Magic link→联结魔法、curative→疗愈、heal→治疗、Break Link→断链、debuff→减益、missile→飞弹、autonomous→自主、potion making→药剂制作；Forage→采集 vs Harvest→收获（分治备案）；Close Combat→近身格斗 vs Melee Fight→近战格斗 vs Range Fight→远程战斗；Kami 知识族 X派 Kami 知识（进攻/治疗/狩猎/守护派）；Kamic Shield→灵能护盾（承 3A 暂译）；Gift→恩赐、Fever→脑热/肌热/神经热、Withering→X觉枯萎、Steal→汲息/汲命/汲树液（汲血同族）、Bullyragging→激怒、Backdraft→回燃、Rondache/Mandragore/Sylve 留拉丁；p 列=name 译（中文无复数）、冠词列 ia/da/pia/pda 全空；desc 无句号随 en；"Tunnels of Oblvion" 拼写错按语义正译（遗忘之隧）。

**本批暂译待复核**：霸战者(Juggernaut)/碎颅者/拳斗家/搏击者（风味称号）；火烙者(Firebrands)/源泉强盗(Bandit of the Source)/Chiang 强者；司祭(Chaplain)、守土者(Landkeeper)、布道统领(Preacher Lord)、会市岔路(Fairmarket Fork)、贝壳之穴(The Concha)、死蝇平原、Varynx 幽窟/出没系列；博爱军团(Force of Fraternity)、树妖部落(Dryads)、蜂群之患(the Swarming)；胶质洼地/胶质洼（Spots of Goo / Goo Spot 两形）；Impact 译"冲击"（与砸击分治备案）。
